...
- Правило ревью пользователя от 2026-09-25: блокирующие замечания публиковать в review PR; неблокирующие замечания оформлять отдельными GitHub issues, предварительно проверив отсутствие дублей. В review не смешивать блокеры с рекомендациями из backlog.
...
- ID `<буква><номер>` принадлежит только волне графа `docs/waves/graph.json`. Оперативные исправления и задачи вне графа не используют ID волн: ветка `codex/ops-<slug>`, план `docs/plans/OPS-<slug>/` (правило 22.09.2026 по #42, пример — `OPS-stage-media-recovery`, бывший PR #32).
