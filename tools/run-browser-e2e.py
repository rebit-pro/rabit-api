...
def test_live(state):
    ...
    output = docker("exec", state["id"] + "-fpm", "php", "/app/tools/e2e/verify-avatar.php", log=Path(state["report"], "avatar-integration.log"))
    if "B3 avatar integration passed" not in output:
        raise RuntimeError("Avatar integration did not complete")
