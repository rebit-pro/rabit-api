...
- Активный граф и историческая карта — `docs/waves/a7/graph.json`; канонический полный план — соседний MoreFoto `docs/04-bitrix-modules/backend-waves.json`. Замороженные `docs/waves/w00` не переписывать. При изменении графа обновить генераторы/карты и прогнать проверку DAG, 99 API ID и независимой готовности.
