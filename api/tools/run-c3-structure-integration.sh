#!/usr/bin/env bash
# C3 native structure/calendar/access Bitrix/MySQL verifier; disposable database only. Run from any directory; no .env is loaded.
set -euo pipefail
...
fixture_id="$(date +%s)-$$"
network="rabit-c3-tests-$fixture_id"
container="rabit-c3-mysql-$fixture_id"
network_created=0
container_created=0
...
if docker container inspect "$container" >/dev/null 2>&1 || docker network inspect "$network" >/dev/null 2>&1; then
    printf 'Refusing to reuse existing C3 container/network. Remove only your stale fixture explicitly.\n' >&2
    exit 1
fi
docker image inspect "$mysql_image" >/dev/null
docker image inspect "$php_image" >/dev/null

docker network create --internal --label rabit.fixture=c3 "$network" >/dev/null
network_created=1
...
docker run --detach --cpus=1 --memory=1g --memory-swap=1g --name "$container" --label rabit.fixture=c3 \
