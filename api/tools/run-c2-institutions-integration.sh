#!/usr/bin/env bash
# C2 native HTTP/assignment Bitrix/MySQL verifier; reuses the unchanged main fixture. Run from any directory; no .env is loaded.
set -euo pipefail
...
network=rabit-c2-tests
container=rabit-c2-mysql
network_created=0
container_created=0
...
if docker container inspect "$container" >/dev/null 2>&1 || docker network inspect "$network" >/dev/null 2>&1; then
    printf 'Refusing to reuse existing C2 container/network. Remove only your stale fixture explicitly.\n' >&2
    exit 1
fi
docker image inspect "$mysql_image" >/dev/null
docker image inspect "$php_image" >/dev/null

docker network create --internal --label rabit.fixture=c2 "$network" >/dev/null
network_created=1
...
docker run --detach --name "$container" --label rabit.fixture=c2 \
...
docker run --rm --read-only --network "$network" --tmpfs /tmp:rw,nosuid,size=256m \
    ...
    tools/verify-c2-institutions-api.php "$@" \
