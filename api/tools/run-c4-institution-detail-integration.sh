#!/usr/bin/env bash
# C4 native institution detail/access Bitrix/MySQL verifier; disposable database only. Run from any directory; no .env is loaded.
set -euo pipefail
...
php_image="${W02_PHP_IMAGE:-rabit-api-php-fpm:20260911-074507}"
mysql_image="${W02_MYSQL_IMAGE:-mysql:8.0}"
fixture_id="$(date +%s)-$$"
network="rabit-c4-tests-$fixture_id"
container="rabit-c4-mysql-$fixture_id"
network_created=0
container_created=0
...
if docker container inspect "$container" >/dev/null 2>&1 || docker network inspect "$network" >/dev/null 2>&1; then
    printf 'Refusing to reuse existing C4 container/network. Remove only your stale fixture explicitly.\n' >&2
    exit 1
fi
docker image inspect "$mysql_image" >/dev/null
docker image inspect "$php_image" >/dev/null

docker network create --internal --label rabit.fixture=c4 "$network" >/dev/null
network_created=1
...
docker run --detach --cpus=1 --memory=1g --memory-swap=1g --name "$container" --label rabit.fixture=c4 \
...
docker run --rm --cpus=2 --memory=1536m --memory-swap=1536m --read-only --network "$network" --tmpfs /tmp:rw,nosuid,size=256m \
    --entrypoint php \
    ...
    tools/verify-c4-institution-detail.php "$@" \
