#!/usr/bin/env bash
# Disposable real Bitrix/MySQL catalogue verifier. Run from any directory; no .env is loaded.
set -euo pipefail

api_root="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd -P)"
kernel_root="${E1_KERNEL_ROOT:-/home/user/rebit-p2p/api/public/bitrix}"
vendor_root="${E1_VENDOR_ROOT:-/home/user/rebit-p2p/api/vendor}"
php_image="${E1_PHP_IMAGE:-rabit-api-php-cli:20260911-074507}"
mysql_image="${E1_MYSQL_IMAGE:-mysql:8.0}"
network=rabit-e1-tests
container=rabit-e1-mysql
network_created=0
container_created=0
...
if docker container inspect "$container" >/dev/null 2>&1 || docker network inspect "$network" >/dev/null 2>&1; then
    printf 'Refusing to reuse existing E1 container/network. Remove only your stale fixture explicitly.\n' >&2
    exit 1
fi
# Docker cannot create the nested vendor mountpoint through a read-only source bind.
# This ignored empty directory holds no dependencies; the actual vendor mount is read-only.
mkdir -p "$api_root/vendor"
docker image inspect "$mysql_image" >/dev/null
docker image inspect "$php_image" >/dev/null

docker network create --internal --label rabit.fixture=e1 "$network" >/dev/null
network_created=1
...
docker run --detach --name "$container" --label rabit.fixture=e1 \
    --network "$network" --network-alias rabit-w02-mysql --mount type=tmpfs,destination=/var/lib/mysql \
...
docker run --rm --read-only --network "$network" --tmpfs /tmp:rw,nosuid,size=256m \
    ...
    tools/verify-e1-catalog-integration.php "$@" \
