...
kernel_root="${E2_KERNEL_ROOT:-/home/user/rebit-p2p/api/public/bitrix}"
vendor_root="${E2_VENDOR_ROOT:-/home/user/rebit-p2p/api/vendor}"
php_image="${E2_PHP_IMAGE:-rabit-api-php-cli:20260911-074507}"
mysql_image="${E2_MYSQL_IMAGE:-mysql:8.0}"
network=rabit-e2-tests
container=rabit-e2-mysql
network_created=0
container_created=0
...
if docker container inspect "$container" >/dev/null 2>&1 || docker network inspect "$network" >/dev/null 2>&1; then
    printf 'Refusing to reuse existing E2 container/network. Remove only your stale fixture explicitly.\n' >&2
    exit 1
fi
...
docker network create --internal --label rabit.fixture=e2 "$network" >/dev/null
network_created=1
...
docker run --detach --name "$container" --label rabit.fixture=e2 \
...
docker run --rm --read-only --network "$network" --tmpfs /tmp:rw,nosuid,size=256m \
    ...
    tools/verify-e2-catalog-api.php "$@" \
