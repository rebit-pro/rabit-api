...
import { CaseValidationError, saveCase } from './service';
export function useCaseEditor(saved: () => void) {
  const auth = useAuthStore(),
    command = ref<CaseCommand | null>(null),
    busy = shallowRef(false),
    error = shallowRef(''),
    errors = shallowRef<CaseErrors>({}),
    restored = shallowRef(false);
  let factory: (() => CaseCommand) | null = null,
    key = '',
    alive = true;
  onScopeDispose(() => {
    alive = false;
  });
  function open(create: () => CaseCommand, context: string) {
    if (busy.value) return;
    factory = create;
    key = 'morefoto:demo:curator-draft:' + auth.user?.id + ':' + context;
    const fresh = create();
    let draft: CaseCommand | null = null;
    try {
  ...
  async function save() {
    ...
    errors.value = {};
    try {
      const copy = structuredClone(JSON.parse(JSON.stringify(command.value))) as CaseCommand;
      await saveCase(auth.getAccessToken() ?? '', copy);
      ...
      if (alive) {
        ...
        if (cause instanceof CaseValidationError) errors.value = cause.errors;
        busy.value = false;
        await nextTick();
        (
