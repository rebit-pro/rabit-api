...
- Открытые решения пользователя:
  - п. 2 — режим Xdebug по умолчанию (варианты A/B/C в плане, рекомендуется A);
...
| T08 | PASS | 2026-09-25 | `docker run --rm --network none --mount type=bind,source=<worktree>/api/docker/development/php/conf.d/xdebug.ini,target=/usr/local/etc/php/conf.d/xdebug.ini,readonly --entrypoint php rabit-api-php-{cli,fpm}:d1-local -r …` | Xdebug загружен, нет `already loaded` и `[Log Files]`; с `XDEBUG_MODE=off` — без предупреждений. Остаётся `[Step Debug] Could not connect` — это режим по умолчанию, решение п. 2 |
