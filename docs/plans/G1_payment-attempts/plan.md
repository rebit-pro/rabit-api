...
### Замечания ревью PR #80 (25.09.2026) и решения

- R1 (P1): отказ повторного `POST /v3/payments` не закрывает `unknown`-попытку. Окончательно закрывает только подтверждённый исход провайдера или отказ самого первого запроса создания (источник START, `CHECK_COUNT=0`, нет ID провайдера). Решение пересчитывается под блокировкой: если параллельная сверка уже сохранила платёж, запоздалый отказ ничего не стирает. Причина в реестре — `provider_refused_retry`.
- R2 (P2): на странице возврата у `pending` с `redirectUrl` есть действие «Продолжить оплату». Панель заказа ведёт туда кнопкой «Проверить или продолжить оплату».
- R3 (P2): ключ идемпотентности привязан к способу оплаты. После неопределённого исхода (сеть, 5xx, 408, 429) другие способы заблокированы, пока PAY-03 не покажет, создана ли попытка. Повтор того же способа идёт с тем же ключом.
- R4 (P2): итог 0 ₽ не оплачивается: PAY-03 `canPay=false`, PAY-01 — 409 `NOTHING_TO_PAY` до SQL и провайдера; экран — «оплата не требуется».
- R5 (P2): E2E не зависит от языка страницы ЮKassa: `locale ru-RU`, отправка формы через `button[type=submit]`, успех — по URL `/v2/success`, возврат — ссылка на наш `return_url`. Шаг с отклоняемой картой убран (платёж остаётся `pending` до срока страницы), вместо него проверяется уход со страницы ЮKassa и продолжение оплаты.
- Не блокирует: polling страницы возврата — issue #83.
- База обновлена на `main` `c8a69a7`: миграция G1 переименована в `Version20260925190001` (номер `20260925120001` занят E6), E6 отмечена слитой в графе.
...
| G1-T19 | Таймаут первого создания, затем 401 повтора; параллельная сверка сохранила платёж до запоздалого отказа | Попытка остаётся `unknown`/`pending`, ID провайдера сохранён, заказ не `declined`, новая попытка запрещена | unit `PaymentReconcilerTest` |
| G1-T20 | Ушёл со страницы ЮKassa до оплаты | Заказ → «Проверить или продолжить оплату» → «Оплата не завершена» → «Продолжить оплату» → оплата той же попыткой | unit `returnState`, E2E G1-T14 |
| G1-T21 | Итог заказа 0 ₽ | PAY-03 `canPay=false`, PAY-01 409 `NOTHING_TO_PAY` без SQL и провайдера | unit |
| G1-T22 | Потерян ответ PAY-01 | Тот же способ — тот же ключ; другой способ — новый ключ только после PAY-03 | unit `startKey`/`isUncertain` |
