...
- Сейчас: commit, push, PR. Merge в `main` выполняется сразу после PR по разрешению пользователя от 2026-09-23 («после PR можно мержить main»), чтобы параллельные ветки получили новый runner.
