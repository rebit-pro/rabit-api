...
Статус: план на согласовании; продуктовый код не начат. Результат этого PR — план, затем изменение графа.
...
| MAX-D07 | Писать из кабинета могут `teacher` и `head` (заведующая). Предложение: обе роли | **Открыто**, блокирует начало K3 |
| MAX-D08 | Ответ принимается от любого участника группы MAX; список разрешённых MAX-пользователей — позже. Предложение: так и оставить в MVP | **Открыто**, блокирует начало K3 |
...
| Бот: «Чат-боты» → «Создать»; логотип 500×500, название «МореФото — вопросы куратору», описание до 200 символов; модерация до 48 ч по рабочим дням | Пользователь | Логотип подготовлен; создание и модерация — в работе |
...
- Предлагается `K1.dependsOn += K3`: K1 расширит модуль Support, а не создаст второй. Решение фиксируется при
  изменении графа.
- Изменение графа — в этом PR после согласования плана: волна K3 в `docs/waves/graph.json`, канонический
  `../MoreFoto/docs/04-bitrix-modules/backend-waves.json` и `docs/05-rest-api/endpoints.json` (SUP-07…12),
  перегенерация карт, `python3 tools/verify-wave-graph.py docs/waves/graph.json`.
...
- [ ] Согласовать план и открытые решения MAX-D07, MAX-D08.
- [ ] Добавить K3 и SUP-07…12 в граф, канонический план и реестр API; прогнать проверку DAG.
...
| --- | --- | --- | --- |
| DOC-01 | План готов → diff от `origin/main` | Только документы плана, затем граф/реестры; runtime-кода нет | `git diff --name-only origin/main...HEAD` |
| DOC-02 | Источники MAX → открыть страницы | Требования к профилю, боту, webhook совпадают с планом | Открытие dev.max.ru |
| DOC-03 | K3 добавлена → проверить граф | DAG корректен, покрытие API учитывает SUP-07…12, K3 ready | `python3 tools/verify-wave-graph.py docs/waves/graph.json` |
