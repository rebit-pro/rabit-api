...
- **D5.** Загрузчик защищённых превью:
  ...
  - смена или очистка access token сразу отменяет запросы прежней сессии и сбрасывает кеш (review PR #60, P1). Запоздалый 401 такого запроса иначе прошёл бы через общий interceptor и сбросил новую сессию.
...
| T18 | Live, превью в полёте; выход и новый вход; задержанный запрос отвечает 401 | Сценарий «#55: превью прежней сессии…» | Запрос отменён при выходе (`net::ERR_ABORTED`), новая сессия и страница каталога сохраняются | live E2E `zz-media.spec.ts` |
