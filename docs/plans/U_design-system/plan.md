...
- DX-B3-U2 (PHPUnit). `ImageResponse`: заголовки кеша и ETag, `If-None-Match` (в том числе список и `W/`) → 304 без тела; `RequestImageToDtoMapper`: без multipart, без файла или с двумя файлами — ошибка; граница `StaffAvatarController` (архитектурная проверка чистого контроллера).
