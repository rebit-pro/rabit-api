...
- DX-U6-01 (финальный gate). Live-спека `zz-overview`: organizer — 4 плитки INF-01 совпадают с `meta.summary` HND-01, очередь и распределение с sr-only таблицей, финансовые плитки без цифр; teacher — карточки групп с countdown; при `reducedMotion: 'reduce'` переходы виджетов нулевые; плитки над списками сотрудников и заказов совпадают со сводками и фильтруют список.
