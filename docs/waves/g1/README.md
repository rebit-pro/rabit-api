...
Полный `make test-e2e` на `0825de7` (стенд `rabit-e2e-47da504c16c6`) — PASS: 109 браузерных сценариев (группа a — 64, группа b — 45), 9 verifier, включая `verify-payments.php`, тестовый магазин ЮKassa включён. Спецификация `zzzzzzzzz-payments`:
- покупатель: отказ СБП тестовым магазином, уход со страницы ЮKassa и «Продолжить оплату», оплата картой на настоящей странице, возврат и «Заказ оплачен»;
- отказы повтора и доступ без ключа, реестр и карточка (organizer; head/teacher — 403, без токена — 401), повтор уведомления, фильтр `late`, desktop/mobile.

Скриншоты — `screenshots/`, `visual.json`; сводка проверок — `verification.json`; тест-кейсы и прогоны — `docs/plans/G1_payment-attempts/progress.md`.
